...
def solve_puzzle(initial_board):
    ...
    visited = set()
    queue = deque([(initial_board, [], 0)])  # Armazena o estado do tabuleiro, o caminho até o estado atual e o custo acumulado

    start_time = time.time()

    while queue:
        current_board, current_path, cost = queue.popleft()
        ...
        for move in moves:
            ...
            if new_board is not None and tuple(map(tuple, new_board)) not in visited:
                ...
                new_cost = cost + 1 + manhattan_distance(new_board)  # custo acumulado + custo do movimento + heurística
